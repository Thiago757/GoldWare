...
import { Search, Star, X, File, ChevronsLeft, ChevronsRight } from 'lucide-react';

const API_BASE_URL = 'http://localhost:3001';

// --- CONFIGURAÇÃO AJUSTADA COM OS RELATÓRIOS QUE VOCÊ PEDIU ---
const relatoriosConfig = [
  ...
  },
  {
    ...
    tags: ['Vendas', 'Financeiro'],
    parametros: [
      { name: 'mes', label: 'Mês de Referência', type: 'month', required: true },
    ],
  },
  {
    ...
    parametros: [
      { name: 'material', label: 'Material Principal', type: 'select', options: ['Todos', 'Ouro 18k', 'Prata 925', 'Outros'] },
    ],
  },
  {
    id: 'estoque_baixo',
  ...
  },
  {
    ...
    titulo: 'Lista de Clientes com Histórico',
  ...
  },
  {
    ...
    parametros: [
        { name: 'status', label: 'Status', type: 'select', options: ['Todas', 'Em Aberto', 'Vencidas'] },
    ],
  },
  { // RELATÓRIO ADICIONADO
    ...
    parametros: [
        { name: 'status', label: 'Status', type: 'select', options: ['Todas', 'A Pagar', 'Vencidas', 'Pagas'] },
...
// --- O RESTANTE DO COMPONENTE CONTINUA EXATAMENTE IGUAL ---
                    ...
                    <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4 max-h-[70vh] overflow-y-auto">
                        ...
                        {report.parametros.map(param => (
                            <div key={param.name} className="md:col-span-2">
                                ...
                                {param.type === 'select' ? (
                                    <select id={param.name} name={param.name} value={params[param.name]} onChange={handleInputChange} required={param.required} className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500"><option value="">Selecione</option>{param.options.map(opt => <option key={opt} value={opt}>{opt}</option>)}</select>
                                ) : (
                        ...
                        <div className="flex items-center space-x-4">
                            {report.formatos.map(f => (
                                <label key={f} className="flex items-center cursor-pointer">
                                    ...
                                    <span className="ml-2 text-sm text-gray-700 capitalize flex items-center">{f}</span>
                                </label>
                            ))}
                        </div>
                    </div>
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-40 p-4">
            <div className="bg-white rounded-lg shadow-2xl w-full max-w-5xl flex flex-col transform transition-all h-full max-h-[800px]">
                <header className="flex justify-between items-center p-4 border-b flex-shrink-0"><h3 className="text-lg font-semibold text-gray-900">Assistente (F4)</h3><button onClick={onClose} className="text-gray-400 hover:text-gray-600"><X size={24} /></button></header>
                ...
                <footer className="p-3 border-t flex justify-end items-center gap-4 flex-shrink-0"></footer>
            </div>
        </div>
    );
};
...
export default function ReportManager() {
  const { isManagerOpen, closeReportManager } = useReports();
  const [selectedReport, setSelectedReport] = useState(null);
  
  const handleSelectReport = (report) => setSelectedReport(report);
  const handleCloseParams = () => setSelectedReport(null);
  
  const handleExecute = (params, format) => {
    if (!selectedReport) return;
    const queryParams = new URLSearchParams({ formato: format, ...params });
    const url = `${API_BASE_URL}${selectedReport.endpoint}?${queryParams.toString()}`;
  ...
  useEffect(() => {
    if (!isManagerOpen) setSelectedReport(null);
  }, [isManagerOpen]);
  ...
  return (
    <>
      {!selectedReport ? (
        <ListModal onSelect={handleSelectReport} onClose={closeReportManager} />
      ) : (
        <ParamsModal report={selectedReport} onClose={handleCloseParams} onExecute={handleExecute} />
      )}
    </>
  );
} 