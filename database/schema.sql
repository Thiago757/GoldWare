...
CREATE TABLE movimentos_bancarios (
    id_movimentobancario SERIAL PRIMARY KEY,
    id_movto INT REFERENCES contas_bancarias(id_conta_bancaria),
    id_recebimento INT REFERENCES recebimentos(id_recebimento),
    id_pagamento INT REFERENCES pagamentos(id_pagamento)
    tipo_movto VARCHAR(10) NOT NULL CHECK(tipo_movto IN ('Entrada', 'Saída')),
    valor NUMERIC(6,2) NOT NULL,
    data_movto DATETIME NOT NULL,
    status_conciliacao NOT NULL CHECK(status_conciliacao IN ('Conciliado', 'Nao_Conciliado'))
);
